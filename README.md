...
Project Workflow
Data Loading — Load the CSV dataset and inspect its shape and structure.
Data Cleaning — Check for missing values, remove duplicates, and filter outliers using Z-scores (threshold = 3).
Exploratory Analysis — Generate a correlation heatmap to understand feature relationships.
Preprocessing — Scale all features using StandardScaler to ensure equal contribution to distance calculations.
Parameter Tuning (Elbow Method) — Test k = 1 to 10 and plot SSE to identify the optimal number of clusters.
Cluster Validation (Silhouette Score) — Compute silhouette scores for k = 2 to 10 to confirm the cluster quality.
K-Means Clustering — Fit the final model with k = 3 and assign cluster labels.
PCA Visualization — Reduce to 2 dimensions and plot clusters to assess separation visually.
Cluster Interpretation — Compare average feature values and wine quality scores per cluster.
Technologies Used
Python 3
...
PCA Cluster Plot
![PCA Cluster Plot]! image

Cluster Feature Comparison
![Cluster Feature Comparison] image
...
Upload WineQT.csv when prompted by the file upload cell.
Run all cells in order (Runtime → Run all).
Option 2 — Local Jupyter
Clone or download this repository.
Install dependencies:
pip install pandas numpy matplotlib seaborn scikit-learn scipy
Place WineQT.csv in the same directory as the notebook.
Remove or comment out the Google Colab upload block and replace it with:
...
└── README.md                     
The dataset can be downloaded from Kaggle — Wine Quality.
